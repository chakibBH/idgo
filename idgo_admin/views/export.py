...
class Export(View):
    ...
    def handle(self, request, *args, **kwargs):
        ...
        if not user.is_authenticated:  # is_authenticated est préférable à is_anonymous
            ...
            if mode == 'all':
                ...
                datasets = Dataset.default.filter(editor=user)
            elif mode == 'ckan_harvested':
                datasets = Dataset.harvested_ckan
            elif mode == 'csw_harvested':
                datasets = Dataset.harvested_csw
            else:
                raise Http404()
            datasets = get_filtered_datasets(datasets, qs)
